/**
 * Full lesson content for the 44 English sounds — server-only on purpose.
 *
 * This array used to live in pronunciation-content.ts, which client routes
 * import for types/SKILLS/PRACTICE_STEPS; because pronunciation.tsx also
 * imported `SOUND_COUNT = PHONEMES.length`, the whole array (every locked
 * sound's how-to, mouth positions, words, sentences and minimal pairs) was
 * bundled into /assets/pronunciation-content-*.js and readable by anyone,
 * regardless of plan — verified on production 2026-09-16. The `.server.ts`
 * suffix keeps it out of the client graph by construction: the only way a
 * learner receives this content now is getSoundsCatalogue, which redacts
 * locked sounds (Yêu cầu 5, Yêu cầu 10).
 */
import type { Phoneme } from "./pronunciation-content";
...
/** The free allowance itself comes from billing_plans.limits
 * ("pronunciation_sounds_free_count") via entitlements.server.ts#getLimits —
 * the caller passes it in. */
export function isSoundIndexFree(index: number, freeCount: number): boolean {
  return index >= 0 && index < freeCount;
}

/** Canonical position in PHONEMES (or -1), matching either the RP or the
 * American symbol, so a caller can check the free allowance. */
...
/** Every practice line /pronunciation actually offers for one sound — each
 * word, each sentence, and each minimal pair in the exact "a. b." form the
 * page sends. analysePronunciation only scores a target from this set, so a
 * free sound's id can't be paired with a locked sound's words. */
export function soundPracticeTargets(index: number): string[] {
  const sound = PHONEMES[index];
  if (!sound) return [];
  return [...sound.words, ...sound.sentences, ...sound.pairs.map((pair) => `${pair.a}. ${pair.b}.`)];
}
